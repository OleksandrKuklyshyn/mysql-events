...
const ZongJi = require("maria-binlog-listener");
...
class MySQLEvents extends EventEmitter {
  ...
  _handleConnectionEvents() {
    this.connection.on("error", (err) =>
      this.emit(EVENTS.CONNECTION_ERROR, err)
    );
  }
  ...
  addTrigger({ name, expression, statement = STATEMENTS.ALL, onEvent }) {
    ...
    if (typeof onEvent !== "function")
      throw new Error("onEvent argument should be a function");

    this.expressions[expression] = this.expressions[expression] || {};
    this.expressions[expression].statements =
      this.expressions[expression].statements || {};
    this.expressions[expression].statements[statement] =
      this.expressions[expression].statements[statement] || [];

    const triggers = this.expressions[expression].statements[statement];
    if (triggers.find((st) => st.name === name)) {
      throw new Error(
        `There's already a trigger named "${name}" for expression "${expression}" with statement "${statement}"`
      );
    }
